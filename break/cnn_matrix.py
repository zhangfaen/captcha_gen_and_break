...
def martix_metrics(y_true, y_pred):
    num = 4
    metrics = {}
    _y = K.equal(K.argmax(y_true), K.argmax(y_pred))
...
def get_cnn_net(num, _shape):
    ...
    y = Reshape((num, CLASS_NUM))(y)
    model = Model(input=inputs, output=y)
    plot(model, show_shapes=True, to_file='cnn_matrix.png')
    model.compile(loss='categorical_crossentropy',
                  loss_weights=[1.],
                  optimizer='Adam',
                  metrics=[martix_metrics])
    return model
